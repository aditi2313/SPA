...
std::unique_ptr<ast::AssignNode> Parser::ParseAssign(int line) {
  auto var_node =
      std::make_unique<ast::VarNode>(ast::VarNode(lexer_->get_ident()));
...
std::unique_ptr<ast::ExprNode> Parser::ParseExpr() {
  ...
  auto lhs = std::make_unique<ast::VarNode>(ast::VarNode(lexer_->get_ident()));

  int op = GetNextTok();

  GetNextTok();

  auto rhs = std::make_unique<ast::VarNode>(ast::VarNode(lexer_->get_ident()));
...
std::unique_ptr<ast::FactorNode> Parser::ParseFactor() {
  ...
  if (current_tok_ == kTokIdent) {
    return std::make_unique<ast::VarNode>(ast::VarNode(lexer_->get_ident()));
  } else if (current_tok_ == kTokInteger) {
    return std::make_unique<ast::ConstNode>(
...
std::unique_ptr<ast::ExprNode> Parser::ParseExpr(std::string expr) {
  ...
  auto lhs = std::make_unique<ast::VarNode>(ast::VarNode(tokens[0]));
  auto op = tokens[1][0];
  auto rhs = std::make_unique<ast::VarNode>(ast::VarNode(tokens[2]));
