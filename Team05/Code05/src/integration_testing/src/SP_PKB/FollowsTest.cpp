...
TEST_CASE("Test SP and PKB integration for Follows data") {
  SECTION("There should be no data if there is only one statement") {
    ...
    auto expected_ftr =
        std::make_unique<filter::FollowsPredicateFilter>([](pkb::FollowsData data){ return true; });
    ...
    REQUIRE(actual_results->get_result()->get_row(0).get_follows_list() ==
            expected_results->get_result()->get_row(0).get_follows_list());
  }
  ...
  SECTION(
      "Follows(s1, s3) holds at the same nesting level in an if statement") {
    std::string program =
        "procedure readfollows { if (x < 0) then { read y; } else { read z; } "
        "}";
    std::unique_ptr<pkb::PKBRelationTable> expected_results =
        std::make_unique<pkb::PKBRelationTable>();
    std::unique_ptr<pkb::PKBRelationTable> actual_results =
  ...
  SECTION(
      "Follows(s1, s3) holds at the same nesting level in two while loops") {
    std::string program =
  ...
  SECTION("Follows(s1, s2) holds in different procedures") {
    std::string program =
