...
class AssignFilterByExpression
    : public IndexableFilter<std::shared_ptr<pkb::AssignData>> {
 public:
  explicit AssignFilterByExpression(std::unique_ptr<ast::ExprNode> expr) {
    expr_ = std::move(expr);
  }
 ...
 private:
  std::unique_ptr<ast::ExprNode> expr_;
};

class AssignFilterByVariable
    : public IndexableFilter<std::shared_ptr<pkb::AssignData>> {
 public:
  explicit AssignFilterByVariable(std::string variable) : variable_(variable) {}
